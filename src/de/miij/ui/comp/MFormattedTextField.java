...
public class MFormattedTextField extends MTextField
{
	...
	/**
	 * Diese Methode &uuml;berpr&uuml;ft, ob die in dem Textfeld stehende Eingabe
	 * korrekt ist. Leerzeichen sind dabei immer erlaubt. Ist eine
	 * Eingabe nicht korrekt, so wird diese mit einem Leerzeichen
	 * ersetzt und der Cursor davor gesetzt.
	 */
	public void check()
	{
		for( int i = 0; i < text().length(); i++ )
		{
			...
			if( !text.equals( " " ) )
			{
				switch( format )
				{
					case DAY 		: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case MONTH 		: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case YEAR 		: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case MINUTE 	: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case SECOND		: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case NUMBER 	: if( !NumberUtil.isInteger( text ) ) fehler( i ); break;
					case ALPHA		: if( NumberUtil.isLong( text ) || NumberUtil.isDouble( text ) ) fehler( i ); break;
	...
	/**
	 * Diese Methode entfernt den an pos stehenden 
	 * Character aus dem Textfeld und ersetzt diesen
	 * mit einem Leerzeichen, setzt anschlie&szlig;end den
