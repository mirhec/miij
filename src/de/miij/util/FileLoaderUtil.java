...
/**
 * Diese Klasse stellte statische Methoden zur Verf&uuml;gung, 
 * welche das dynamische Laden von (zur Laufzeit nicht bekannten)
 * Klassen m&ouml;glich machen.
 * Es ist a&ouml;ch m&ouml;glich, andere EXEs zu starten.
 * 
 * @author Mirhec
 */
		...
		try
		{
			...
			if( f.isFile() )
			{
				if( filename.substring( filename.length() - 3 ).equals( "jar" ) ) // Nur *.jar-Files
				{
					...
					{ f.toURL() } ).loadClass( filename.substring( 0 , filename.length() - 4 ) );
